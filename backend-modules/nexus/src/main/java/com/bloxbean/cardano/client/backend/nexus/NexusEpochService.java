...
public class NexusEpochService implements EpochService {
    ...
    private ProtocolParams toProtocolParams(adlabs.nexus.client.backend.api.epoch.model.ProtocolParams pp) {
        ...
        // Upstream CCL also carries a costModelsRaw field with no Nexus SDK source (the SDK has only the
        // map-of-map form); this fork has no such field, so there is nothing to leave null.
