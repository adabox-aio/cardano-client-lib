...
* `AccountService::getAccountAddressTotal` — Nexus exposes no received/sent totals per stake account
* `TransactionService::getDelegationCertificates` — Nexus declares a `certificates` field on its
  transaction payload but never populates it (every provider mapper ignores the target)
...
* `UtxoService::getAllUtxos` and `AssetService::getAssetInformationBulk` have no single-shot Nexus
  endpoint; both are emulated client-side (page-until-short-page and per-unit loop respectively).
* `TransactionCbor::utxo` and `TransactionCbor::txSize` have no Nexus source and stay null.
  `txTimestamp` arrives as a stringified Unix epoch-seconds value and is parsed defensively —
  anything non-numeric maps to null rather than failing the transaction.

## Fork note

This module was ported from bloxbean/cardano-client-lib master, where it is not yet part of any
released version (it landed after `v0.8.0-pre5` and appears only in the unpublished `v0.8.0-pre6-dev*`
tags). This fork's backend interfaces are a superset of upstream's, so the port adds implementations
for `getAllUtxos`, `getAssetInformationBulk`, `getTransactionWithdrawals`, `getTransactionCbor`,
`getTransactionsCbor`, `getAccountAddressTotal` and `getDelegationCertificates`, none of which exist
upstream.
